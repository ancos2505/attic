...
impl CartPresenter {
    pub(crate) async fn count(
        ...
        let carts_count = Count::presenter(&Count, sqlite_pool, ()).await?;
        let header_value = match HeaderValue::from_str(&carts_count) {
            ...
            Err(error) => {
                ...
                return Err((
                    StatusCode::CONFLICT,
                    "Error on serialization counted Carts.".to_string(),
                ));
            }
        };
